import { mergeLayers } from "../core/Layer.ts";
import type { ILayer, ILayerLifecycles } from "../core/Layer.types.ts";
import type { Tools } from "../core/Tools.ts";
...
export function createGroup<Child extends ILayer>(
  { children: initialChildren }: IGroupOptions<Child> = {},
): IGroup<Child> {
  ...
  function updateMergeLifecycles() {
    mergedLifecycles = mergeLayers(
      ...children.mapSafe((child) => mountedChildren.get(child) ?? {}),
    );
  }
